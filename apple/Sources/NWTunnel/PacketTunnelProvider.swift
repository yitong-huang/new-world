...
/// Network Extension entry: add this target to Xcode as **Packet Tunnel Provider** and link `NWTunnel`.
open class PacketTunnelProvider: NEPacketTunnelProvider {
    ...
    private func recvSome(_ conn: NWConnection) async throws -> Data {
        try await withCheckedThrowingContinuation { (cont: CheckedContinuation<Data, Error>) in
            conn.receive(minimumIncompleteLength: 1, maximumLength: 65536) { data, _, isComplete, err in
                ...
                if isComplete { cont.resume(throwing: NSError(domain: "NWTunnel", code: 5, userInfo: nil)); return }
            }
        }
    }
    ...
    private func popFrame(_ buffer: inout Data) throws -> NwFrame? {
        ...
        let (t, ln) = try NwFraming.decodeHeader(buffer.prefix(NwFraming.headerSize))
