// 与 apple/Sources/NwVpnWire/Framing.swift 保持一致；修改协议时请同步两处。
import Foundation
...
public enum NwFraming {
    ...
    public static func decodeHeader(_ prefix: Data) throws -> (NwMsgType, Int) {
        ...
        // Data slices can keep a non-zero start index after removeFirst/subdata operations.
        // Normalize before integer indexing so parsing the next buffered frame cannot trap.
        let b = [UInt8](prefix.prefix(headerSize))
        if b[0] != 0x4E || b[1] != 0x57 || b[2] != 0x30 || b[3] != 0x31 {
            throw NwFramingError.badMagic
        }
        let ver = (UInt16(b[4]) << 8) | UInt16(b[5])
        if ver != 1 { throw NwFramingError.badVersion }
        guard let t = NwMsgType(rawValue: b[6]) else { throw NwFramingError.badType }
        if b[7] != 0 { throw NwFramingError.badVersion }
        let len = Int(
            (UInt32(b[8]) << 24)
                | (UInt32(b[9]) << 16)
                | (UInt32(b[10]) << 8)
